...
async function handleAuth() {
  ...
  try {
    ...
    localStorage.setItem('username', data.username.replace('@', ''));
  ...
  if (!matchDeck.length || currentDeckIndex >= matchDeck.length) {
    container.innerHTML = `
      <div class="glass-card p-6 rounded-3xl text-center space-y-2">
        ...
        <p class="text-xs text-slate-400">No more users nearby right now.</p>
...
// Isolated Messaging System
async function openChat(target) {
  activeTarget = target.replace('@', '');
  document.getElementById('activeChatTitle').innerText = target.startsWith('grp_') ? '🌐 Lounge Squad' : `@${activeTarget}`;
  loadConversations();
...
socket.on('receive_direct_message', (msg) => {
  ...
  const cleanRecipient = msg.recipient.replace('@', '');

  if (isGroup && cleanRecipient === activeTarget) {
    appendChatMessage(cleanSender, msg.text, cleanSender === currentHandle);
  } else if (!isGroup) {
...
async function fetchPosts() {
  ...
  feed.innerHTML = posts.map(p => `
    <div class="glass-card p-4 rounded-2xl space-y-2">
      ...
      <button onclick="saveDM('${p.authorName}'); openMatchedChat('${p.authorName}')" class="text-[11px] bg-indigo-600/30 border border-indigo-500/30 text-indigo-300 px-3 py-1 rounded-xl">Silent DM 👋</button>
    </div>
  `).join('');
}
